...
def loads(xmlString, viewName, flashOk=True, viewDigest=None, sourceApp=None):
    ...
    if root.tag == 'dashboard' and root.get('version') == '2':
        ...
        v1_template = util.getDashboardV1p1TemplateUri() if root.get('version') == '1.1' else util.getDashboardV1TemplateUri()
...
class MainTest(unittest.TestCase):
    def test_loads_dashboard_v1_root_is_dashboard(self):
        ...
        self.assertEqual(viewConfig, { 'template': '/pages/dashboard.html', 'simplexml': b'<dashboard></dashboard>', 'isSimpleXML': True, 'modules': {} })

    def test_loads_dashboard_v1_root_is_form(self):
        viewConfig = loads('<form></form>', 'fake_dashboard_id')
        self.assertEqual(viewConfig, { 'template': '/pages/dashboard.html', 'simplexml': b'<form></form>', 'isSimpleXML': True, 'modules': {} })

    def test_loads_dashboard_v1p1_root_is_dashboard(self):
    ...
    def test_loads_dashboard_v1p1_root_is_form(self):
