...
class ControllerMetaLoader(object):
    ...
    def load_module(self, full_name):
        ...
        # Try to compile, so we can catch syntax errors
        try:
            code = compile(code, file_path, "exec")
        except SyntaxError as e:
            logger.error("{file_path} contains Python syntax errors: {e}".format(file_path=file_path, e=e))
            return
        ...
        try:
            ...
            logger.error("{file_path} cannot be executed by Python: {e}".format(file_path=file_path, e=e))
            return
