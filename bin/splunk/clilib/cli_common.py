...
def mergeConfFile(targetConfFilename, sourceStanzas):
    """
    Write source config stanzas to a target config file. If the target already
    contains config, merge the source config into it.
    """
    target_stanzas = readConfFile( targetConfFilename )
    if not target_stanzas is None and len(target_stanzas) > 0:
        target_stanzas = _multi_update(target_stanzas, sourceStanzas)
    else:
        target_stanzas = sourceStanzas
    writeConfFile(targetConfFilename, target_stanzas)
...
class Tests(unittest.TestCase):
  ...
  def test_merge_conf_with_mixed_stanzas(self):
      source_stanzas = self._create_conf_file_and_read_it(
          b"[default]\n"
          b"key1 = value1\n")
      expected_stanzas = self._create_conf_file_and_read_it(
          b"[default]\n"
          b"key1 = value1\n"
          b"key2 = value2\n")
      actual_stanzas = {}
      try:
          target_stanzas = {"default": {"key2": "value2"}}
          fd, target_conf_file = tempfile.mkstemp()
          os.closerange(fd, fd + 1)
          writeConfFile(target_conf_file, target_stanzas)
          mergeConfFile(target_conf_file, source_stanzas)
          actual_stanzas = readConfFile(target_conf_file)
      finally:
          os.unlink(target_conf_file)
      self.assertEqual(actual_stanzas, expected_stanzas)

  def test_merge_conf_with_diff_stanzas(self):
      source_stanzas = self._create_conf_file_and_read_it(
          b"[foo]\n"
          b"key1 = value1\n")
      expected_stanzas = self._create_conf_file_and_read_it(
      ...
      actual_stanzas = {}
      try:
          target_stanzas = {"bar": {"key2": "value2"}}
          fd, target_conf_file = tempfile.mkstemp()
          os.closerange(fd, fd + 1)
          writeConfFile(target_conf_file, target_stanzas)
          mergeConfFile(target_conf_file, source_stanzas)
  ...
  def test_merge_conf_with_no_target(self):
      source_stanzas = self._create_conf_file_and_read_it(
          b"[default]\n"
          b"key1 = value1\n")
      expected_stanzas = self._create_conf_file_and_read_it(
